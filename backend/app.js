...
const app = express();
const PORT = process.env.PORT || 3000; // Assurez-vous que ce port est bien celui que vous utilisez pour le backend
...
const corsOptions = {
    ...
    // C'est la cause du problème "Failed to fetch" lorsque credentials: true est utilisé avec '*'.
    // Si votre frontend peut être accédé depuis http://localhost:5173 ET http://10.87.63.23:5173,
    // utilisez un tableau comme ceci:
    origin: ['http://10.87.63.23:5173', 'http://localhost:5173'],
    // Si seulement l'IP est utilisée:
...
cron.schedule('0 16 * * *', async () => { // MODIFIÉ
    ...
    try {
        ...
        const previousHistoricalDayDateOnly = dateForArchivingPeriod.toISOString().split('T')[0]; // Format-MM-DD
        ...
        try {
            ...
            // --- PHASE 3 (Optionnel but recommended) : ARCHIVAGE (Snapshot de debut de journee historique) ---
...
(async () => {
    try {
        ...
        app.listen(PORT, host, () => { // Ajout de 'host' comme deuxième argument
            console.log(`Serveur backend démarré sur http://${host}:${PORT}`);
            // Afficher l'adresse locale pour un accès facile depuis d'autres appareils
            console.log(`Accessible sur le réseau local via http://10.87.63.23:${PORT}`); // Remplacez 10.87.63.23 par votre IP si elle change
